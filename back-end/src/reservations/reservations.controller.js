...
const validateBody = (req, res, next) => {
  ...
  requiredFields.forEach((requirement) => {
    if (
      !req.body.data.hasOwnProperty(requirement) ||
      req.body.data[requirement] === ""
    )
  ...
  if (reservationDate < today)
    return next({
      status: 400,
      message:
        "Please choose either today or a future date to schedule your visit",
    });
  ...
  if (Number.isNaN(Date.parse(req.body.data.reservation_date))) {
    return next({
      status: 400,
      message: "reservation_date field is in an incorrect format",
    });
  }

  //test if reservation_time is in HH:MM format
  if (
    !/^([0-1][0-9]|[2][0-3]):([0-5][0-9])$/.test(req.body.data.reservation_time)
  ) {
    return next({
      status: 400,
      message: "reservation_time field is in an incorrect format",
    });
  }

  //test if reservation_time bounds are not crossed
  if (
    req.body.data.reservation_time < "10:30" ||
    req.body.data.reservation_time > "21:30"
  ) {
    return next({
      status: 400,
      message:
        "Please choose a reservation time within the working hours of the restaurant",
    });
  }
  ...
  if (
    ...
    return next({
      status: 400,
      message: "Please select a future reservation time",
    });
  }
  ...
  if (parseInt(req.body.data.people) < 1)
    return next({
      status: 400,
      message: "people must be greater than or equal to 1",
    });
  ...
  if (req.body.data.status && req.body.data.status !== "booked")
    return next({
      status: 400,
      message: `Reservations must always start off booked, not ${req.body.data.status}`,
    });
...
const validReservation = async (req, res, next) => {
  if (!req.body.data.reservation_id)
    return next({ status: 404, message: `Please input a reservation` });
  const checkIfExists = await service.read(req.body.data.reservation_id);
  if (!checkIfExists)
    return next({
      status: 404,
      message: `The reservation with the id ${req.body.data.reservation_id} does not exist`,
    });
  next();
};
...
const canUpdate = async (req, res, next) => {
  ...
  if (!originalReservation[0])
    return next({
      status: 404,
      message: `The reservation with the id ${reservation_id} does not exist`,
    });
  if (originalReservation[0].status === "finished")
    return next({
      status: 400,
...
async function read(req, res, next) {
  try {
    ...
    if (!reservation[0])
      return next({
        status: 404,
        message: `A reservation with the id ${reservation_id} does not exist`,
      });
...
async function list(req, res) {
  try {
    let response;
    if (req.query.hasOwnProperty("mobile_number"))
      response = await service.search(req.query.mobile_number);
