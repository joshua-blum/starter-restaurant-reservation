...
const validateBody = (req, res, next) => {
  ...
  if (req.body.data.table_name.split("").length <= 1)
    return next({
      status: 400,
      message: `The table_name must be longer than 1 character`,
    });
  if (req.body.data.capacity === 0)
    return next({
      status: 400,
      message: "The table must not have a capacity of zero",
    });
...
const canUpdate = async (req, res, next) => {
  ...
  if (!originalTable || !originalTable[0])
    return next({
      status: 404,
      message: `There does not exist a table with the id ${table_id}`,
    });
  ...
  if (res.locals.table.capacity < res.locals.reservation.people)
    return next({
      status: 400,
      message: `The table only has capacity for ${res.locals.table.capacity} spaces, which is not enough space for the ${res.locals.reservation.people} people in the reservation`,
    });

  if (res.locals.table.reservation_id)
    return next({
      status: 400,
      message: `The table is already occupied by the reservation with the following id: ${res.locals.table.reservation_id}`,
    });

  const alreadySeated = await service.checkIfReserved(
    res.locals.reservation.reservation_id
  );
  if (alreadySeated[0])
    return next({
      status: 400,
      message: `The reservation with id ${res.locals.reservation.reservation_id} is already seated`,
    });
  next();
};
...
const canUnassignReservation = async (req, res, next) => {
  ...
  if (!res.locals.table.reservation_id)
    return next({
      status: 400,
      message: `The table with id ${res.locals.table.table_id} is not occupied`,
    });
  ...
  if (!reservation || !reservation[0])
    return next({
      status: 404,
      message: `The reservation with the id ${res.locals.table.reservation_id} does not exist`,
    });
...
const hasValidReservation = async (req, res, next) => {
  ...
  if (!req.body.data.reservation_id)
    return next({
      status: 400,
      message: "To seat you, we need your reservation_id",
    });
  //ensure that the reservation_id belongs to an actual reservation
  const reservation = await reservationService.read(
