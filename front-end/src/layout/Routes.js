...
function Routes() {
  ...
  const reservationAssignment = async (reservation_id, table_id, signal) => {
    const updatedTable = await assignReservation(reservation_id, table_id, signal);
    return updatedTable;
  };
  ...
  const reservationStatusChange = async (reservation_id, newStatus, signal) => {
    const updatedReservation = await updateReservationStatus(reservation_id, newStatus, signal);
    return updatedReservation;
  }

  const reservationUpdate = async (modifiedReservation, id, signal) => {
    const updatedReservation = await editReservation(modifiedReservation, id, signal);
    return updatedReservation;
  }
